...
app.post('/api/register', async (req, res) => {
  ...
  if(!name || !entryNo || !password) return res.status(400).json({error: "All fields required"});
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    if (user && await bcrypt.compare(password, user.password)) {
      const token = jwt.sign({ id: user._id }, process.env.JWT_SECRET, { expiresIn: '24h' }); // Token expiry is good practice
      res.json({ token, name: user.name, registeredEvents: user.registeredEvents });
    } else {
...
app.post('/api/book', async (req, res) => {
  ...
  try {
    ...
    await User.findByIdAndUpdate(decoded.id, { $addToSet: { registeredEvents: eventId } });
